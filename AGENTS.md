...
`CARGO_TARGET_DIR=/tmp/cz_cursor_cargo_target`; the hook uses
`/tmp/cz_full_check_cargo_target` (lld empty-note compile fails on a shared
`target/`). Keep tracey links intact
