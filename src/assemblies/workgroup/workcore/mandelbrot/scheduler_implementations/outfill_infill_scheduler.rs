    ...
    fn pick_step(&mut self) -> Option<((i32, i32), Step)> {
        ...
        for step in try_order {
            ...
            if let Some((pos, st)) = found {
                let rank = st.preference_rank();
                if self.phase_jobs.suspended_rank() == Some(rank) {
                    // Higher-preference work drained; resume the interrupted phase.
                    self.phase_jobs.resume_suspended();
                } else {
                    self.phase_jobs.consider_incoming(rank);
                }
                return Some((pos, st));
            }
        }
        // No queued work this pass — clear preempt bookkeeping.
        self.phase_jobs.resume_suspended();
        None
    }
...
mod d_sch3_tests {
    ...
    #[test]
    fn phase_job_tracker_resume_restores_suspended() {
        let mut t = PhaseJobTracker::idle();
        t.start(Step::In.preference_rank());
        assert!(t.consider_incoming(Step::Out.preference_rank()));
        t.resume_suspended();
        assert_eq!(t.active_rank(), Some(Step::In.preference_rank()));
        assert_eq!(t.suspended_rank(), None);
    }

    #[test]
    fn pick_step_resumes_suspended_when_higher_drains() {
        let mut state = OutfillInfillScheduler::init_for_tile_extent((8, 8));
        // Screen-edge gate open; clear other queues so only In is eligible.
        state.tile_edge_remaining = 0;
        state.scredge.clear();
        state.out_queue.clear();
        state.edge_queue.clear();
        state.in_filament_edge_queue.clear();
        state.out_filament_edge_queue.clear();
        state.period_edge_queue.clear();
        state.flood_in_queue.clear();
        state.small_time_edge_queue.clear();
        state.in_queue.clear();
        state.in_queue.push_back(((1, 1), 0));
        state.phase_jobs.start(Step::In.preference_rank());
        assert!(state.phase_jobs.consider_incoming(Step::Out.preference_rank()));
        assert_eq!(state.phase_jobs.suspended_rank(), Some(Step::In.preference_rank()));
        let picked = state.pick_step();
        assert!(matches!(picked.map(|(_, s)| s), Some(Step::In)));
        assert_eq!(state.phase_jobs.active_rank(), Some(Step::In.preference_rank()));
        assert_eq!(state.phase_jobs.suspended_rank(), None);
    }
