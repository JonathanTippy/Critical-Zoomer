...
mod mutant_kill {
    ...
    fn relative_copy_intexp1_mag_44_does_not_f64_collapse_c() {
        use crate::assemblies::headgroup::window::coords::{decimal_str_to_intexp, ul_for_center};
        use crate::copy_intexp::CopyIntExp1;
        use super::{from_stencil_with_margin, DirectKernel, SeatKernel};
        let res = crate::constants::DEFAULT_WINDOW_RES;
        let loc = ul_for_center(
            decimal_str_to_intexp("-0.1146689120911964").unwrap(),
            decimal_str_to_intexp("0.9695042757337979").unwrap(),
            44,
            res,
        );
        let mut ctx = from_stencil_with_margin::<CopyIntExp1>((loc, res), None, 1, true)
            .expect("mag 44 CopyIntExp<1>");
        assert!(
            ctx.coords_are_relative,
            "mag 44 headed path is relative CopyIntExp"
        );
        let corners = [
            (0i32, 0i32),
            (res.0 as i32 - 1, 0),
            (0, res.1 as i32 - 1),
            (res.0 as i32 - 1, res.1 as i32 - 1),
            (res.0 as i32 / 2, res.1 as i32 / 2),
        ];
        let mut uniq = Vec::new();
        for pos in corners {
            DirectKernel.start_seat(&mut ctx, pos);
            let idx = crate::utils::index_from_pos(&pos, ctx.res.0);
            let c = ctx.points[idx].c;
        ...
        assert!(
            uniq.len() > 1,
            "relative naive must not collapse plane c ({} unique of 5 corners; f64(c) may still match)",
            uniq.len()
        );
    }
