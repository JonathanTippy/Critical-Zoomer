...
- **Depth integration — final gear push (2026-08-07).** Live actors stay on f64 host seats; mathematical deltas use the F64 → ScaledF64 → FloatExp compute gear ladder (`src/delta_gear.rs`) with live series on `PublishedReference`. Earlier all-FloatExp live experiment (646b847..565b078) caused Xvfb banding and was reverted; banding root cause for FloatExp *host* remains open but is moot for the f64-host + gear-compute path. Finish-line gates (home parity, scaled-f64 win, series skip, ≥2^3600000 capacity, HUD gear+IPS+PPS) must pass together — see `design/depth-design.md` Acceptance.
...
- **Headgroup/shadergroup test strategy** (open problem): the workgroup now has property tests bound to its craftsmanship rules; the headgroup does not. The screenshot harness is the only net for visual bugs but needs use on every edit and image-description trust is imperfect; oracles can rot when output legitimately changes; the only known visual property so far is real-axis reflection symmetry. Needs a stronger strategy before the GPU shade port — the shadergroup was cut back last time partly for lack of tests. In-app PPM snip (`snip.rs` / `CZ_SNIPREQ`) is a start for faux-user paths.
