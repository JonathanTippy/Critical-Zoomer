...
- **Color gear (OG ↔ GPU) — design lock, not built (2026-08-11).** When a GPU
  colorer exists, settings get a manual switch like the screen-worker gear:
  OG CPU colorer vs GPU, no auto-pick. GPU path must meet the honest-rewrite
  parity bar (`shadergroup-virtues.md`, `assembly-boundaries.md`). Not started.
