...
- **Depth integration — f64 live path restored (2026-08-07).** Earlier `WorkContext<FloatExp>` live experiment (646b847..565b078) produced severe Xvfb banding (~40–120 black columns vs 0 baseline) and was reverted. Live actors use f64; FloatExp remains only in test-only depth kernels. Root cause of live failure remains open: isolated pipeline tests passed while live actor timing/channel behavior differed.
