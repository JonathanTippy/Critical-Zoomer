...
- **PPS-selected kernel (naive vs pert)** (`r[cz.perf.pps-selected-kernel+1]`): **landing** — view-start PPS race among legal kernels (Naive / Naive GPU / Pert); no GPU-first assumption; lock highest measured PPS (`pps_probe_locks_highest_measured_kernel`).
