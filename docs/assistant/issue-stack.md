...
- **Perturbation performance design decision (blocking ≤20% gate).** Same-path FloatExp delta iteration on f64-valid home is ~5.7–5.9× slower than the accepted direct baseline after honest opts. Closing to ≤20% without a second production kernel needs an explicit developer decision (representation/codegen of FloatExp), not a zero-orbit special case into `iterate_max_n_times`.
