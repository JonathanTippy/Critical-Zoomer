...
**Charter note (2026-08-12 eye tracking):** headgroup `inputs` already owns pointer attention. Adding webcam gaze + four-corner toast calibration, and a second workgroup attention spiral from the gaze point (mixmap slot 3). Unobtrusive HUD toast (“look at the top-left corner”), not a full-screen calibration overlay.
