...
slider render headroom (default 1 bit)**. Same margin on absolute `c` and
perturbation `delta_c`. OG naive CPU: f32, then f64, then `CopyIntExp<1>` after
the absolute f64 wall. No mid-iteration “upgrade gear” path: new stencil →
