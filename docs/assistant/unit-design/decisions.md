...
| D-SERIES-1 | Series approximation is **live** (design + implement). Performance contract in `r[cz.depth.series-approximation+1]` / `D-SERIES-2`…`6`. |
...
| D-PUB-2 | Calibrated→answer bias: clamp the proximate value into the proven range, field by field. All answer fields treated as numeric (no non-numeric special case). **Living (2026-08-12):** skip vs v0.0.9. Bias would feel better if publishing were slow; the benefit is too small to justify the complexity. Target is PPS through whole-snapshot publish; when PPS flows, this would not be noticed. Do not port as a standing path. (`r[cz.int.publisher-nores-bias+1]`) |
| D-PUB-3 | Publisher is **continuity of output only**. Tile completion and TPS must **not** depend on the publisher. |
| D-PUB-4 | GPU-native path: worker exposes a **GPU-resident calibrated tile**; publisher binds it **directly** (uploader bypass). Tile-era text added proximate→GPU Answer bias. **Living (2026-08-12):** skip that bias pass — nicer on a slow pipe, not worth the complexity; v0.0.9 is whole-snapshot remap + Dummy honesty. Uploader exists only for CPU-completed calibrated tiles. |
