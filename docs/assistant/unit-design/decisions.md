...
| D-PUB-2 | Calibrated→answer bias: clamp the proximate value into the proven range, field by field. All answer fields treated as numeric (no non-numeric special case). **Living (2026-08-12):** overbuilt vs v0.0.9. Target is PPS through whole-snapshot publish, not a bias synthesizer for a trickle of new data. If slow, honest publishing feels better; when PPS flows, this would not be noticed. Do not port as a standing path. (`r[cz.int.publisher-nores-bias+1]`) |
| D-PUB-3 | Publisher is **continuity of output only**. Tile completion and TPS must **not** depend on the publisher. |
| D-PUB-4 | GPU-native path: worker exposes a **GPU-resident calibrated tile**; publisher binds it **directly** (uploader bypass). Tile-era text added proximate→GPU Answer bias. **Living (2026-08-12):** that bias pass is overbuilt; v0.0.9 is whole-snapshot remap + Dummy honesty, not a calibrated→Answer synthesizer. Uploader exists only for CPU-completed calibrated tiles. |
