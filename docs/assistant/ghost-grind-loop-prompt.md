...
names, **or code** after the project became assisted at v0.0.9.
Implementation ghosts (wrong edits) count; they take git history to find.
Developer
...
```
Ghost grind. A ghost is an assistant misunderstanding in comments, docs, names, or code since v0.0.9. Implementation ghosts (assistant edit mistakes) count toward the total; hunt them in git history. Evolved developer ideas are candidate ghosts — deference to hard-won hand-coded design; Trash spec-dictator docs are history, not law.
...
4. If the tick touched src/, benches, Cargo, or Tracey rules, `scripts/full_check.sh` is the gate (release full suite + benches + tracey). Docs-only ticks skip the suite. Checkpoint: automatic checkpoint <datetime>.
