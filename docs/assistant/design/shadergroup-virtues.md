...
channel between actors) measured dummy-head GPU **esc ~60 Hz**. **Headed HUD
not re-checked** after that pass — do not call live rates fixed until the
window shows it. Shipping/host round-trip (map wait) remains.
