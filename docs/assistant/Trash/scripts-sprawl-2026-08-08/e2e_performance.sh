...
# Perfect: after ~100ms of zoom-in, frame must be structured (not stalled empty).
# Wall time includes sleep+fifo+import and is diagnostic only — do not gate on it.
e2e_send "zoomin 2"
sleep 0.1
...
echo "simple_zoom_100ms_stdev=$SIMPLE_STDEV (min 1500; product bar 100ms intent)"
if [ "$SIMPLE_STDEV" -ge 1500 ]; then
  e2e_pass "simple zoom structured after 100ms intent stdev=$SIMPLE_STDEV"
else
  e2e_fail_msg "simple zoom flat after 100ms intent stdev=$SIMPLE_STDEV"
fi
