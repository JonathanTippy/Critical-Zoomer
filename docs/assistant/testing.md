...
| **Pipeline cadence (dummy head)** | `assemblies/pipeline.rs` (`steady_state_pipeline_cadence_*`) | Real workgroup+shadergroup graph; dummy head fans settings/stencil and records the same `pub:`/`esc:`/`col:` RateCounters. OG and GPU escape both require content-class `esc` (≥15 OG / ≥40 GPU). Dummy-head GPU pin went green 2026-08-12 after convert hoist + same-walk GPU prepack (~60 Hz); headed HUD still the live check |
...
`steady_state_pipeline_cadence_og_escape` and `_gpu_escape` should stay green
after the 2026-08-12 convert/prepack work. Headed `esc:` with `escape:GPU` is
still the product check (dummy-head is not the window).
