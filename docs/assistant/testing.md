...
| **Pipeline cadence (dummy head)** | `assemblies/pipeline.rs` (`steady_state_pipeline_cadence_*`) | Real graph + dummy head HUD rates. Floors: OG esc ≥15, GPU esc ≥40. **Ghost-hunt 2026-08-12:** those tests **fail** on current debug (~10 OG / ~23 GPU). Older “dummy-head GPU ~60 Hz, test currently passes” is stale. Headed HUD is the product check — not declared fixed. Do not ignore or lower the floors in this hunt (GPU grind paused). |
...
**serialized** and prefer **release** so GPU color floors are honest.
**Ghost-hunt 2026-08-12:** debug run failed both OG and GPU floors (see
`ghost-hunt-2026-08-12.md`). Do not treat “currently pass” as true. Headed
`esc:` with `escape:GPU` is still the product check.
