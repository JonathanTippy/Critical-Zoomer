...
4. REGRESSION GATE (non-negotiable — do not skip or narrow):
   - Full gate: `scripts/full_check.sh` (unit then integration then e2e, then benches + Tracey). Fix every failure you introduced or uncovered before checkpointing. “Only the tests I think are relevant” is forbidden.
